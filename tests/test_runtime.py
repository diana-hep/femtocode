...
class TestRuntime(unittest.TestCase):
    ...
    def test_explode(self):
        ...
        self.assertEqual(Dataset(xss=collection(collection(integer)), ys=collection(integer)).fromPython(xss=[[[100, 200], [300, 400], [500, 600]]], ys=[[1, 2, 3, 4]]).toPython("xss.map(xs => xs.map(x => ys.map(y => x + y)))").run(), [[[[101, 102, 103, 104], [201, 202, 203, 204]], [[301, 302, 303, 304], [401, 402, 403, 404]], [[501, 502, 503, 504], [601, 602, 603, 604]]]])
        ...
        self.assertEqual(Dataset(xss=collection(collection(integer)), ys=collection(integer)).fromPython(xss=[[[100, 200], [300, 400], [500, 600]]], ys=[[1, 2, 3, 4]]).toPython("xss.map(xs => ys.map(y => xs.map(x => x + y)))").run(), [[[[101, 201], [102, 202], [103, 203], [104, 204]], [[301, 401], [302, 402], [303, 403], [304, 404]], [[501, 601], [502, 602], [503, 603], [504, 604]]]])
